...
import { User as UserIcon, Key, Trash2, Plus, ChevronLeft, ChevronRight, Search, BookOpen, RotateCcw, Eye } from 'lucide-react';
...
export default function StudentSection({ onUpdate }: StudentSectionProps) {
  ...
  const loadFilterData = useCallback(async () => {
    try {
      const tutorsRes = await adminAPI.getTutors();
                      ...
                      <td className="py-2 pr-4">
                        <div className="flex gap-1">
                          <Button type="button"
                            size="sm"
                            variant="outline"
                            onClick={() => setDetailModal({ open: true, item: student })}
