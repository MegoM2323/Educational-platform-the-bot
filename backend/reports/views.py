...
from .serializers import (
    ...
    TutorWeeklyReportSerializer,
    TutorWeeklyReportCreateSerializer,
    TeacherWeeklyReportSerializer,
    TeacherWeeklyReportCreateSerializer,
...
class ReportViewSet(viewsets.ModelViewSet):
    ...
    def generate(self, request, pk=None):
        ...
        if report.status != Report.Status.DRAFT:
            return Response(
                {"error": "Отчет уже сгенерирован"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def send(self, request, pk=None):
        ...
        for student in report.target_students.all():
            recipients.append(
                ReportRecipient.objects.create(report=report, recipient=student)
            )

        for parent in report.target_parents.all():
            recipients.append(
                ReportRecipient.objects.create(report=report, recipient=parent)
            )
...
class StudentReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "parent":
            ...
            children = User.objects.filter(
                student_profile__parent_id=user.id, role="student"
    ...
    def my_children(self, request):
        ...
        if request.user.role != "parent":
            return Response(
                {"error": "Требуется роль родителя"}, status=status.HTTP_403_FORBIDDEN
            )

        # Get children from StudentProfile
        children = User.objects.filter(
            student_profile__parent_id=request.user.id, role="student"
        ).values_list("id", flat=True)
        ...
        reports = (
            ...
            .select_related("teacher", "student", "parent")
            .order_by("-created_at")
        )
    ...
    def acknowledge(self, request, pk=None):
        ...
        return Response(
            {"message": "Отчет подтвержден", "report": serializer.data},
    ...
    def generate_progress_report(self, request):
        ...
        try:
            report_input = CreateStudentReportInput(
                ...
                progress_percentage=int(
                    progress_data["summary"]["overall_progress_percentage"]
                ),
            )
        ...
        return Response(
            {"report": serializer.data, "progress_data": progress_data},
    ...
    def get_student_progress(self, request):
        ...
        if request.user.role == "teacher":
            ...
            is_parent_of_student = User.objects.filter(
                id=student_id, student_profile__parent_id=request.user.id
            ).exists()
...
class ReportTemplateViewSet(viewsets.ModelViewSet):
    ...
    queryset = ReportTemplate.objects.select_related(
        "created_by", "parent_template", "previous_version"
    )
    ...
    def validate_layout(self, request, pk=None):
        ...
        try:
            ...
            return Response(
                {
                    "message": "Layout configuration is valid",
...
class ReportScheduleViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role in ["teacher", "tutor", "admin"]:
            return ReportSchedule.objects.filter(created_by=user).prefetch_related(
                "recipient_entries"
            )
        else:
    ...
    def remove_recipient(self, request, pk=None):
        ...
        try:
            recipient_entry = ReportScheduleRecipient.objects.get(
                schedule=schedule, recipient_id=recipient_id
            )
    ...
    def executions(self, request, pk=None):
        ...
        return Response(
            {"count": schedule.executions.count(), "results": serializer.data}
        )
    ...
    def my_subscriptions(self, request):
        ...
        subscriptions = ReportScheduleRecipient.objects.filter(
            recipient=request.user, is_subscribed=True
    ...
    def unsubscribe(self, request):
        ...
        token = request.data.get("unsubscribe_token") or request.query_params.get(
            "unsubscribe_token"
        )
                ...
                return Response(
                    {
                        ...
                        "schedule_name": recipient_entry.schedule.name
                        or f"Schedule {recipient_entry.schedule.id}",
                    },
                    status=status.HTTP_200_OK,
                )
            else:
...
class TutorWeeklyReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "tutor":
            ...
            if children_ids:
                ...
                return qs.filter(parent_id=user.id).exclude(
    ...
    def create(self, request, *args, **kwargs):
        ...
        output_serializer = TutorWeeklyReportSerializer(
    ...
    def send(self, request, pk=None):
        ...
        if report.status != TutorWeeklyReport.Status.DRAFT:
            return Response(
                {"error": "Отчет уже отправлен"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        return Response(
            {"message": "Отчет отправлен родителю", "report": serializer.data},
            status=status.HTTP_200_OK,
        )
        ...
        if report.parent_id and report.parent_id == request.user.id:
            ...
            try:
                ...
                if (
                    ...
                    and student_profile.parent_id == request.user.id
                ):
                    is_authorized = True
            except StudentProfile.DoesNotExist:
                pass
    ...
    def available_students(self, request):
        ...
        if request.user.role != "tutor":
            return Response(
                {"error": "Требуется роль тьютора"}, status=status.HTTP_403_FORBIDDEN
            )

        students = (
            User.objects.filter(
                student_profile__tutor_id=request.user.id, role="student"
            )
...
class TeacherWeeklyReportViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == "teacher":
            ...
            students_ids = list(
                User.objects.filter(
                    student_profile__tutor_id=user.id, role="student"
                ).values_list("id", flat=True)
            )
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Используем основной сериализатор для возврата полных данных
        instance = serializer.instance
        output_serializer = TeacherWeeklyReportSerializer(
            instance, context={"request": request}
        )
        headers = self.get_success_headers(output_serializer.data)
        return Response(
    ...
    def send(self, request, pk=None):
        ...
        return Response(
            {"message": "Отчет отправлен тьютору", "report": serializer.data},
            status=status.HTTP_200_OK,
        )
    ...
    def mark_read(self, request, pk=None):
        ...
        if report.tutor_id and report.tutor_id == request.user.id:
            ...
            try:
                ...
                if (
                    student_profile.tutor_id
                    and student_profile.tutor_id == request.user.id
                ):
                    is_authorized = True
            except StudentProfile.DoesNotExist:
                pass
    ...
    def available_students(self, request):
        ...
        enrollments = (
            SubjectEnrollment.objects.filter(teacher=request.user, is_active=True)
            .select_related(
                "student",
                "subject",
    ...
    def generate_now(self, request):
        ...
        from reports.services.teacher_weekly_report_service import (
            TeacherWeeklyReportService,
        )
                    ...
                    if subject_id:
                        ...
                        if created_report:
                            serializer = TeacherWeeklyReportSerializer(
                                created_report, context={"request": request}
                            )
    ...
    def by_student(self, request):
        ...
        if not student_id:
            return Response(
                {"error": "Не указан student_id"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            student_profile = StudentProfile.objects.select_related("tutor").get(
                user_id=student_id, tutor_id=request.user.id
            )
        except StudentProfile.DoesNotExist:
            return Response(
        ...
        reports = (
            TeacherWeeklyReport.objects.filter(student_id=student_id)
            .filter(
                Q(tutor_id=request.user.id)
                | Q(student__student_profile__tutor_id=request.user.id)
            )
            .select_related(
                "teacher", "student", "subject", "tutor", "student__student_profile"
            )
            .order_by("-week_start")
        )
...
class ParentReportViewSet(APIView):
    """
    API для получения отчетов родителем.

    GET /api/reports/parent/ - список отчетов ребенка/детей
    Фильтры: child_id, start_date, end_date
    """

    permission_classes = [permissions.IsAuthenticated]

    def get(self, request):
        """Получить список отчетов для родителя"""
        user = request.user
        ...
        try:
            ...
            parent_profile = (
                user.parent_profile if hasattr(user, "parent_profile") else None
            )
            if not parent_profile:
                return Response({"results": [], "count": 0})

            students = StudentProfile.objects.filter(parent=user).values_list(
                "user_id", flat=True
            )
            ...
            queryset = (
                StudentReport.objects.filter(student_id__in=students)
            ...
            if child_id:
                queryset = queryset.filter(student_id=child_id)
            ...
            serializer = StudentReportSerializer(page_obj.object_list, many=True)
            return Response(
                {
            ...
            logger.error(f"Error in ParentReportViewSet.get: {str(e)}")
            return Response({"results": [], "count": 0})
