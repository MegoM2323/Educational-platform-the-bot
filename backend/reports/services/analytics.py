...
class LearningAnalyticsService:
    ...
    def _get_cached_or_calculate(
        self, cache_key: str, calculate_func, *args, **kwargs
    ) -> Dict[str, Any]:
        """
    ...
    def get_student_analytics(self, student_id: int) -> Dict[str, Any]:
        ...
        return self._get_cached_or_calculate(
            cache_key, self._calculate_student_analytics, student_id
        )
    ...
    def _calculate_student_analytics(self, student_id: int) -> Dict[str, Any]:
        ...
        material_progress = MaterialProgress.objects.filter(
            student=student, started_at__gte=period_start
        ).select_related("material")

        # Get assignment data
        assignment_submissions = AssignmentSubmission.objects.filter(
            student=student, submitted_at__gte=period_start
        ...
        assignments_completed = assignment_submissions.exclude(
            status="returned"
        ).count()
        ...
        learning_progress = self._calculate_learning_progress(
            student=student, period_start=period_start
        )
        ...
        # Calculate trend
        trend = self._calculate_trend(student=student, period_start=period_start)
    ...
    def _calculate_engagement_score(
        self, materials, assignments, elements, period_start
    ) -> float:
        """
        ...
        assignment_score = (
            (assignments.exclude(status="returned").count() / assignment_count * 100)
        ...
        activity_score = (
            min(100, (total_activities / days_elapsed) * 10) if days_elapsed > 0 else 0
        )
    ...
    def _calculate_learning_progress(self, student: User, period_start) -> float:
        """
        ...
        enrollments = SubjectEnrollment.objects.filter(
            student=student, enrolled_at__gte=period_start
        ).count()
        ...
        assignment_performance = (
            ...
            .aggregate(avg_score=Avg("score"), count=Count("id"))
        )
    ...
    def _calculate_activity_frequency(
        self, material_progress, assignments, period_start
    ) -> str:
        """
        ...
        material_days = (
            material_progress.values_list(TruncDate("last_accessed"), flat=True)
            .distinct()
            .count()
        )

        assignment_days = (
            assignments.values_list(TruncDate("submitted_at"), flat=True)
            .distinct()
            .count()
        )
    ...
    def _get_last_activity(self, student: User, period_start) -> Optional[str]:
        ...
        last_material = (
            MaterialProgress.objects.filter(
                student=student, last_accessed__gte=period_start
            )
            .latest("last_accessed")
            .last_accessed
            if MaterialProgress.objects.filter(student=student).exists()
            else None
        )
    ...
    def _calculate_trend(self, student: User, period_start) -> str:
        ...
        first_half_engagement = self._calculate_engagement_score(
            materials=MaterialProgress.objects.filter(
                student=student, started_at__gte=period_start, started_at__lt=mid_point
            ),
            assignments=AssignmentSubmission.objects.filter(
                student=student,
                submitted_at__gte=period_start,
                submitted_at__lt=mid_point,
            ),
        ...
        second_half_engagement = self._calculate_engagement_score(
            materials=MaterialProgress.objects.filter(
                student=student, started_at__gte=mid_point, started_at__lt=now
            ),
            assignments=AssignmentSubmission.objects.filter(
                student=student, submitted_at__gte=mid_point, submitted_at__lt=now
            ),
            elements=ElementProgress.objects.filter(
                student=student, started_at__gte=mid_point, started_at__lt=now
            ),
    ...
    def _determine_risk_level(
        self, engagement_score: float, trend: str, activity_frequency: str
    ) -> str:
    ...
    def get_class_analytics(self, class_id: int) -> Dict[str, Any]:
        ...
        return self._get_cached_or_calculate(
            cache_key, self._calculate_class_analytics, class_id
        )
    ...
    def _calculate_class_analytics(self, class_id: int) -> Dict[str, Any]:
        ...
        learning_progress_scores = [
            s["learning_progress"] for s in student_analytics_list
        ]

        avg_engagement = mean(engagement_scores) if engagement_scores else 0
        avg_learning_progress = (
            mean(learning_progress_scores) if learning_progress_scores else 0
        )

        # Calculate at-risk count
        at_risk_count = sum(
            1
            for s in student_analytics_list
            if s["engagement_score"] < self.AT_RISK_THRESHOLD
        )

        # Calculate distribution
        excellent_count = sum(
            1 for s in student_analytics_list if s["engagement_score"] >= 80
        )
        good_count = sum(
            1 for s in student_analytics_list if 60 <= s["engagement_score"] < 80
        )
        fair_count = sum(
            1 for s in student_analytics_list if 40 <= s["engagement_score"] < 60
        )
        poor_count = sum(
            1 for s in student_analytics_list if s["engagement_score"] < 40
        )
        ...
        return {
            ...
            "at_risk_percentage": round(
                at_risk_count / len(student_analytics_list) * 100, 2
            ),
    ...
    def get_subject_analytics(self, subject_id: int) -> Dict[str, Any]:
        ...
        return self._get_cached_or_calculate(
            cache_key, self._calculate_subject_analytics, subject_id
        )
    ...
    def _calculate_subject_analytics(self, subject_id: int) -> Dict[str, Any]:
        ...
        enrollments = (
            SubjectEnrollment.objects.filter(
                subject_id=subject_id, enrolled_at__gte=period_start
            )
        ...
        avg_time_spent = material_progress.aggregate(avg=Avg("time_spent"))["avg"] or 0

        avg_progress = (
            material_progress.aggregate(avg=Avg("progress_percentage"))["avg"] or 0
        )
    ...
    def identify_at_risk_students(
        self, threshold: int = 40, limit: int = 100
    ) -> List[Dict[str, Any]]:
        """
        ...
        for student in students:
            ...
            if (
                ...
                and analytics["engagement_score"] < threshold
            ):
    ...
    def generate_learning_recommendations(self, student_id: int) -> List[str]:
        """
    ...
    def get_batch_student_analytics(
