...
from .models import (
    GraphLesson, LessonElement, Element,
    ElementProgress, LessonProgress, LessonDependency
)
    ...
    try:
        ...
        with transaction.atomic():
            ...
            if element.element_type == 'quick_question' and is_correct is False:
                ...
                elem_progress.attempts = (elem_progress.attempts or 0) + 1
                elem_progress.save(update_fields=['answer', 'score', 'status', 'attempts'])
            else:
            ...
            was_auto_completed, unlocked_lessons = ProgressSyncService.auto_complete_lesson_if_all_elements_done(
                request.user,
                graph_lesson
            )
...
def _unlock_dependent_lessons(user, graph_lesson):
    """
    Разблокировать уроки которые зависят от текущего

    Args:
        user: User объект студента
        graph_lesson: GraphLesson который был завершен

    Returns:
        list: список разблокированных уроков [{"id": int, "title": str}]
    """
    unlocked = []

    try:
        # Получить прогресс завершенного урока
        lesson_progress = LessonProgress.objects.get(
            student=user,
            graph_lesson=graph_lesson
        )
        ...
        ).select_related('to_lesson', 'to_lesson__lesson')

        for dep in dependencies:
            # Проверить выполнено ли условие по баллам
            if lesson_progress.max_possible_score > 0:
                score_percent = (lesson_progress.total_score / lesson_progress.max_possible_score) * 100
            else:
                score_percent = 100
            ...
            if score_percent >= dep.min_score_percent:
                ...
                for other_dep in all_deps:
                    ...
                    # Проверить процент баллов
                    if other_progress.max_possible_score > 0:
                        other_score_percent = (other_progress.total_score / other_progress.max_possible_score) * 100
                    else:
                        other_score_percent = 100

                    if other_score_percent < other_dep.min_score_percent:
                        all_met = False
                        break

                # Если все условия выполнены, разблокировать урок
                if all_met and not dep.to_lesson.is_unlocked:
                    dep.to_lesson.unlock()
                    unlocked.append({
                        'id': dep.to_lesson.id,
                        'lesson_id': dep.to_lesson.lesson.id,
                        'title': dep.to_lesson.lesson.title,
                    })

    except Exception as e:
        logger.error(f"Error in _unlock_dependent_lessons: {e}", exc_info=True)
