...
class KnowledgeGraphSerializer(serializers.ModelSerializer):
    ...
    def validate_student(self, value):
        ...
        if value.role != 'student':
            raise serializers.ValidationError("Указанный пользователь не является студентом")
        return value
...
class AddLessonToGraphSerializer(serializers.Serializer):
    ...
    def validate_node_color(self, value):
        ...
        if not re.match(r'^#[0-9A-Fa-f]{6}$', value):
            raise serializers.ValidationError("Некорректный формат цвета (ожидается #RRGGBB)")
        return value
    ...
    def validate_lessons(self, value):
        ...
        for lesson_data in value:
            if 'lesson_id' not in lesson_data:
                raise serializers.ValidationError("Каждый урок должен содержать lesson_id")

            if 'position_x' not in lesson_data or 'position_y' not in lesson_data:
                raise serializers.ValidationError("Каждый урок должен содержать position_x и position_y")
            ...
            try:
                ...
                raise serializers.ValidationError("position_x и position_y должны быть числами")
