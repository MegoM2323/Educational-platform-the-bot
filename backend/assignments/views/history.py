...
from assignments.serializers_history import (
    ...
    SubmissionVersionListSerializer,
    SubmissionVersionDetailSerializer,
    SubmissionVersionDiffSerializer,
    SubmissionVersionRestoreSerializer,
...
class AssignmentHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            AssignmentHistory.objects.filter(assignment_id=assignment_id)
            .select_related("changed_by", "assignment")
            .order_by("-change_time")
        )

        # Students can only see history for their assigned assignments
        if self.request.user.role == "student":
            queryset = queryset.filter(assignment__assigned_to=self.request.user)
    ...
    def get_object(self):
        ...
        return get_object_or_404(
            AssignmentHistory, id=history_id, assignment_id=assignment_id
        )
...
class SubmissionVersionViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_submission(self):
        ...
        if user.role == "student" and submission.student_id != user.id:
            self.permission_denied(
                self.request, "Cannot view other students' submissions"
            )
            ...
            if submission.student not in children:
                self.permission_denied(
                    self.request, "Cannot view non-child submissions"
                )
    ...
    def get_object(self):
        ...
        return get_object_or_404(
            SubmissionVersion, id=version_id, submission_id=submission_id
        )
    ...
    def diff(self, request, submission_id=None):
        ...
        try:
            version_a = SubmissionVersion.objects.get(
                submission=submission, version_number=int(version_a_num)
            )
            version_b = SubmissionVersion.objects.get(
                submission=submission, version_number=int(version_b_num)
            )
        ...
        if not diff_obj:
            ...
            try:
                diff_obj = SubmissionVersionDiff.objects.create(
                    version_a=version_a, version_b=version_b, diff_content=diff_content
                )
            except Exception as e:
                logger.error(f"Error creating diff: {e}")
                diff_content = {"error": "Failed to generate diff"}

        serializer = SubmissionVersionDiffSerializer(
            diff_obj, context={"request": request}
        )
    ...
    def restore(self, request, submission_id=None):
        ...
        try:
            restore_from_version = SubmissionVersion.objects.get(
                submission=submission, version_number=version_number
            )
    ...
    def restores(self, request, submission_id=None):
        ...
        restores = (
            SubmissionVersionRestore.objects.filter(submission=submission)
            .select_related(
        ...
        serializer = SubmissionVersionRestoreSerializer(
            restores, many=True, context={"request": request}
        )
        return Response(serializer.data)
