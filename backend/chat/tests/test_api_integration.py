...
class TestChatAPIList:
    ...
    def setup_method(self):
        ...
        self.student = User.objects.create(
            username="api_student", password="testpass", role="student"
        )
        self.teacher = User.objects.create(
            username="api_teacher", password="testpass", role="teacher"
        )
...
class TestChatAPICreate:
    ...
    def setup_method(self):
        self.client = APIClient()
        self.student = User.objects.create(
            username="api_student2", password="testpass", role="student"
        )
        self.teacher = User.objects.create(
            username="api_teacher2", password="testpass", role="teacher"
        )

        # Create SubjectEnrollment to allow chat between student and teacher
    ...
    def test_create_chat_invalid_recipient(self):
        ...
        self.client.force_authenticate(user=self.student)
        response = self.client.post("/api/chat/", {"recipient_id": 999999999})
...
class TestChatAPIDetail:
    ...
    def test_get_chat_detail_not_found(self):
        ...
        self.client.force_authenticate(user=self.student)
        response = self.client.get("/api/chat/999999999/")
        assert response.status_code == status.HTTP_404_NOT_FOUND
...
class TestChatAPIMessages:
    ...
    def test_send_message_unauthorized(self):
        ...
        response = self.client.post(
            f"/api/chat/{self.room.id}/send_message/", {"content": "Hello!"}
        )
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_send_message_empty_fails(self):
        ...
        self.client.force_authenticate(user=self.student)
        response = self.client.post(
            f"/api/chat/{self.room.id}/send_message/", {"content": "   "}
        )
        assert response.status_code == status.HTTP_400_BAD_REQUEST
...
class TestChatAPIMessageEdit:
    ...
    def setup_method(self):
        ...
        self.message = MessageService.send_message(self.student, self.room, "Original")
...
class TestChatAPIMessageDelete:
    ...
    def setup_method(self):
        ...
        self.message = MessageService.send_message(self.student, self.room, "To delete")
...
class TestChatAPIRead:
    ...
    def test_mark_as_read_unauthorized(self):
        """Test that unauthenticated requests are rejected"""
        response = self.client.post(f"/api/chat/{self.room.id}/mark_as_read/")
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
...
class TestChatAPICompleteWorkflow:
    ...
    def test_full_chat_workflow(self):
        ...
        # Create SubjectEnrollment to allow chat
        subject = Subject.objects.create(name="Test Subject")
        SubjectEnrollment.objects.create(
            student=student,
            teacher=teacher,
        ...
        response = client.post(
            f"/api/chat/{room_id}/send_message/", {"content": "Hello teacher!"}
        )
        ...
        # 6. Mark as read
        response = client.post(f"/api/chat/{room_id}/mark_as_read/")
        assert response.status_code == 200
