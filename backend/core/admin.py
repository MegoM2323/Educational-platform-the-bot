...
class FailedTaskAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Метаданные", {"fields": ("metadata_formatted",), "classes": ("collapse",)}),
        (
            "Анализ и решение",
            {
                "fields": (
...
class TaskExecutionLogAdmin(admin.ModelAdmin):
    ...
    def status_badge(self, obj):
        """Статус с цветовой индикацией"""
        colors = {"success": "green", "failed": "red", "retrying": "orange"}
        color = colors.get(obj.status, "gray")
        return format_html(
            '<span style="background-color: {}; color: white; padding: 3px 10px; border-radius: 3px;">{}</span>',
            color,
    ...
    def result_formatted(self, obj):
        ...
        return format_html(
            "<pre>{}</pre>", json.dumps(obj.result, indent=2, ensure_ascii=False)
        )
...
class AuditLogAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Действие", {"fields": ("action", "timestamp")}),
        ("Пользователь", {"fields": ("user",)}),
        ("Объект", {"fields": ("target_type", "target_id")}),
        ("Запрос", {"fields": ("ip_address", "user_agent")}),
        ("Метаданные", {"fields": ("metadata_formatted",), "classes": ("collapse",)}),
    )
    ...
    def get_queryset(self, request):
        """Оптимизируем queryset с select_related для админ панели"""
...
class ConfigurationAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("key", "group", "description")}),
        ("Значение", {"fields": ("value_formatted", "value_type")}),
        (
            "История",
            {
    ...
    def value_formatted(self, obj):
        ...
        if isinstance(obj.value, dict):
            return format_html(
                "<pre>{}</pre>", json.dumps(obj.value, indent=2, ensure_ascii=False)
            )
