...
def teacher_user_1(db):
    ...
    user = User.objects.create_user(
        username="teacher_schedule_1",
        email="teacher_schedule_1@test.com",
        password="teacher123",
        first_name="Ivan",
        last_name="Petrov",
...
def teacher_user_2(db):
    ...
    user = User.objects.create_user(
        username="teacher_schedule_2",
        email="teacher_schedule_2@test.com",
        password="teacher456",
        first_name="Maria",
        last_name="Smirnova",
        role=User.Role.TEACHER,
...
def student_user_1(db):
    ...
    user = User.objects.create_user(
        username="student_schedule_1",
        email="student_schedule_1@test.com",
        password="student123",
        first_name="Alex",
        last_name="Ivanov",
...
def student_user_2(db):
    ...
    user = User.objects.create_user(
        username="student_schedule_2",
        email="student_schedule_2@test.com",
        password="student456",
        first_name="Nina",
        last_name="Sokolova",
        role=User.Role.STUDENT,
...
@pytest.fixture
def sample_lessons(db, teacher_user_1, teacher_user_2, student_user_1, student_user_2, subject_math, subject_english):
    """Create sample lessons for testing"""
    # Create TeacherSubject relationships (required for Lesson validation)
    TeacherSubject.objects.create(teacher=teacher_user_1, subject=subject_math, is_active=True)
    TeacherSubject.objects.create(teacher=teacher_user_2, subject=subject_english, is_active=True)

    # Create SubjectEnrollment relationships (required for Lesson validation)
    SubjectEnrollment.objects.create(student=student_user_1, subject=subject_math, teacher=teacher_user_1, is_active=True)
    SubjectEnrollment.objects.create(student=student_user_2, subject=subject_english, teacher=teacher_user_2, is_active=True)
    SubjectEnrollment.objects.create(student=student_user_2, subject=subject_math, teacher=teacher_user_1, is_active=True)
    SubjectEnrollment.objects.create(student=student_user_1, subject=subject_english, teacher=teacher_user_2, is_active=True)
    ...
    lesson4 = Lesson.objects.create(
        ...
        description="English lesson 2 (cancelled)",
    )
...
class TestAdminScheduleList:
    ...
    def test_list_response_contains_required_columns(self, admin_client, sample_lessons):
        ...
        required_fields = [
            ...
            'teacher_name', 'student_name', 'subject_name', 'status'
        ]
...
class TestAdminScheduleCreate:
    ...
    def test_admin_can_create_lesson(self, admin_client, teacher_user_1, student_user_1, subject_math):
        ...
        response = admin_client.post('/api/admin/schedule/lessons/create/', data, format='json')
    ...
    def test_created_lesson_appears_in_list(self, admin_client, teacher_user_1, student_user_1, subject_math):
        ...
        create_response = admin_client.post('/api/admin/schedule/lessons/create/', data, format='json')
        ...
        created_lesson = next(
            (l for l in lessons if l['subject_name'] == 'Mathematics' and '11:00' in l['start_time']),
            None
        )
        assert created_lesson is not None

    def test_create_lesson_requires_teacher(self, admin_client, student_user_1, subject_math):
    ...
    def test_create_lesson_requires_student(self, admin_client, teacher_user_1, subject_math):
    ...
    def test_create_lesson_with_invalid_teacher(self, admin_client, student_user_1, subject_math):
...
class TestAdminScheduleFilter:
    ...
    def test_combined_filters(self, admin_client, sample_lessons, teacher_user_1, subject_math):
...
class TestAdminSchedulePermissions:
    ...
    def test_non_admin_cannot_create_lesson(self, db, teacher_user_1):
        ...
        response = client.post('/api/admin/schedule/lessons/create/', data, format='json')
        assert response.status_code == 403
...
class TestAdminScheduleIntegration:
    ...
    def test_full_schedule_management_workflow(
        ...
        create_response = admin_client.post('/api/admin/schedule/lessons/create/', create_data, format='json')
        assert create_response.status_code == 201
    ...
    def test_schedule_with_multiple_status_changes(
        ...
        response = admin_client.post('/api/admin/schedule/lessons/create/', create_data, format='json')
        assert response.status_code == 201
        ...
        lessons = list_response.json()['results']
        new_lesson = next(
            (l for l in lessons if '16:00' in l['start_time']),
            None
        )
