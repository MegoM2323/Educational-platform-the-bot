...
class DashboardConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        if not self.check_dashboard_access():
            logger.warning(f'[DashboardConsumer] Connection rejected: user {self.user.id} has no dashboard access')
            await self.close()
            return
        ...
        logger.info(f'[DashboardConsumer] User {self.user.id} ({self.user.email}) connected to dashboard')
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(f'[DashboardConsumer] User {self.user.id} disconnected from dashboard (code: {close_code})')
    ...
    def get_initial_metrics(self) -> dict:
        ...
        try:
            ...
            active_students = AssignmentSubmission.objects.filter(
                assignment__in=assignments
        ...
        try:
            while True:
                ...
                if not self.pong_received:
                    missed_pongs += 1
                    logger.warning(f'[DashboardConsumer] User {self.user.id} missed pong #{missed_pongs}')

                    if missed_pongs >= 2:
                        logger.warning(f'[DashboardConsumer] Closing connection for user {self.user.id} (missed 2 pongs)')
                        await self.close()
                        return
                else:
                    missed_pongs = 0
