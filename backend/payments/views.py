            ...
            try:
                if "/" in allowed_network:
                    ...
                    if client_ip_obj in ip_network(allowed_network, strict=False):
                        logger.info(
                            f"YooKassa webhook IP {client_ip} verified (network: {allowed_network})"
                        )
                        return True
                else:
                    # Это конкретный IP
                ...
                logger.error(
                    f"Invalid network in YOOKASSA_WEBHOOK_IPS: {allowed_network}: {e}"
                )
                continue
...
def verify_yookassa_signature(body: bytes, signature: str, secret_key: str) -> bool:
    ...
    try:
        ...
        if is_valid:
            ...
            logger.warning(
                f"YooKassa webhook signature verification failed: expected={expected_signature[:8]}..., got={signature[:8]}..."
            )
...
def create_yookassa_payment(payment, request):
    ...
    if not settings.YOOKASSA_SHOP_ID or not settings.YOOKASSA_SECRET_KEY:
        logger.error(
            "YOOKASSA_SHOP_ID or YOOKASSA_SECRET_KEY not configured. Please set these environment variables."
        )
        return {
    ...
    if "localhost" in frontend_url and request and hasattr(request, "get_host"):
        try:
            ...
            if host and "localhost" not in host and "127.0.0.1" not in host:
                ...
                if (
                    env_config.PRODUCTION_DOMAIN in host
                    or f"www.{env_config.PRODUCTION_DOMAIN}" in host
                ):
                    frontend_url = (
                        f"{protocol}://{host.split(':')[0]}"  # Убираем порт если есть
                    )
                    logger.warning(
                        f"FRONTEND_URL was localhost, but detected production domain. Using {frontend_url}. "
                        f"Please set FRONTEND_URL in .env file for production!"
                    )
                    # Если frontend на другом порту, нужно указать его в FRONTEND_URL в .env
        except Exception as e:
            logger.debug(
                f"Could not determine host from request: {e}, using FRONTEND_URL from settings: {frontend_url}"
            )

    # Проверяем, что на продакшене не используется localhost
    allowed_hosts = getattr(settings, "ALLOWED_HOSTS", [])
    has_production_host = (
        any(env_config.PRODUCTION_DOMAIN in str(h) for h in allowed_hosts)
        if allowed_hosts
        else False
    )
    if has_production_host and "localhost" in frontend_url:
        logger.warning(
            f"WARNING: Production server detected, but FRONTEND_URL is set to localhost: {frontend_url}. "
    ...
    return_url = (
        f"{frontend_url}/dashboard/parent/payment-success?payment_id={payment.id}"
    )

    logger.info(
        f"Using frontend URL for return: {frontend_url}, return_url: {return_url}"
    )
    ...
    try:
        logger.info(
            f"Creating YooKassa payment for payment {payment.id}, amount: {payment.amount}"
        )
        ...
        if response.status_code in [200, 201]:
            ...
            if data:
                ...
                logger.error(
                    f"Failed to parse YooKassa response. Response text: {response.text[:500]}"
                )
                return None
        else:
            ...
            try:
                error_data = (
                    ...
                    else safe_json_response(response)
                )
                if error_data and isinstance(error_data, dict):
                    ...
                    if response.status_code == 401 or response.status_code == 403:
                        ...
                        "invalid" in str(error_description).lower()
                        and "credentials" in str(error_description).lower()
                    ):
                        user_message = "Ошибка конфигурации платежной системы. Обратитесь к администратору."
...
def check_yookassa_payment_status(yookassa_payment_id):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"YooKassa API error: {response.status_code} - {response.text}"
            )
            return None
...
def yookassa_webhook(request):
    ...
    if webhook_signature:
        ...
        if not verify_yookassa_signature(
            request.body, webhook_signature, settings.YOOKASSA_SECRET_KEY
        ):
        ...
        logger.warning(
            "Webhook received without signature (X-Yookassa-Webhook-Signature header missing)"
        )
    ...
    try:
        ...
        with db_transaction.atomic():
            ...
            try:
                ...
                logger.error(
                    f"Payment not found for YooKassa ID: {yookassa_payment_id}"
                )
            ...
            if event_type == "payment.succeeded":
                ...
                if payment.status == Payment.Status.SUCCEEDED:
                    logger.info(
                        f"Payment {payment.id} already processed as SUCCEEDED, skipping"
                    )
                    return HttpResponse("OK")

                # Проверяем тип платежа: invoice payment или subject payment
                invoice_id = (
                    payment.metadata.get("invoice_id")
                ...
                if invoice_id:
                    ...
                    logger.info(
                        f"Detected invoice payment: payment_id={payment.id}, invoice_id={invoice_id}"
                    )
                ...
                logger.info(
                    f"Invalidated YooKassa status cache for payment {payment.yookassa_payment_id}"
                )
                ...
                cache.delete(cache_key)
                logger.info(
                    f"Invalidated YooKassa status cache for canceled payment {payment.yookassa_payment_id}"
                )
                ...
                try:
                    ...
                    for subject_payment in subject_payments:
                        if subject_payment.status in [SP.Status.WAITING_FOR_PAYMENT]:
                            ...
                            logger.info(
                                f"SubjectPayment {subject_payment.id} status changed to PENDING after payment cancellation"
                            )
                except Exception as e:
                    logger.error(
                        f"Failed to update SubjectPayment status after cancellation: {e}"
                    )
                ...
                try:
                    ...
                    for subject_payment in subject_payments:
                        if subject_payment.status == SP.Status.PENDING:
                            ...
                            subject_payment.save(update_fields=["status", "updated_at"])
                            logger.info(
                                f"SubjectPayment {subject_payment.id} status changed to WAITING_FOR_PAYMENT"
                            )
                except Exception as e:
                    logger.error(
                        f"Failed to update SubjectPayment status to WAITING_FOR_PAYMENT: {e}"
                    )
                ...
                cache.delete(cache_key)
                logger.info(
                    f"Invalidated YooKassa status cache for failed payment {payment.yookassa_payment_id}"
                )
                ...
                try:
                    ...
                    for subject_payment in subject_payments:
                        if subject_payment.status in [SP.Status.WAITING_FOR_PAYMENT]:
                            ...
                            subject_payment.save(update_fields=["status", "updated_at"])
                            logger.info(
                                f"SubjectPayment {subject_payment.id} status changed to PENDING after payment failure"
                            )
                except Exception as e:
                    logger.error(
                        f"Failed to update SubjectPayment status after payment failure: {e}"
                    )
...
def check_payment_status(request):
    ...
    try:
        ...
        if payment.status == Payment.Status.SUCCEEDED:
            logger.debug(
                f"Payment {payment.id} already SUCCEEDED in DB, returning immediately (no YooKassa check needed)"
            )
            return JsonResponse(
                {
        ...
        if payment.status in [
            ...
            logger.debug(
                f"Payment {payment.id} in terminal state {payment.status} in DB, returning immediately"
            )
            return JsonResponse(
                {
                    "status": payment.status,
        ...
        if payment.yookassa_payment_id:
            ...
            if yookassa_status is None:
                logger.debug(
                    f"Checking YooKassa API for payment {payment.id} (not in cache)"
                )
                yookassa_status = check_yookassa_payment_status(
                ...
                if yookassa_status:
                    ...
                    logger.debug(
                        f"Cached YooKassa status '{yookassa_status}' for payment {payment.id}"
                    )
            else:
                logger.debug(
                    f"YooKassa status '{yookassa_status}' for payment {payment.id} from cache"
                )
            ...
            if yookassa_status:
                ...
                if (
                    ...
                    and payment.status != Payment.Status.CANCELED
                ):
                    payment.status = Payment.Status.CANCELED
                    payment.save(update_fields=["status", "updated"])
                    # Обновляем статус SubjectPayment обратно на PENDING при отмене
                    try:
                        from materials.models import (
                            SubjectPayment as SubjectPaymentModel,
                        )

                        subject_payments = SubjectPaymentModel.objects.filter(
                            payment=payment
                        )
                        for sp in subject_payments:
                            if sp.status in [
                                SubjectPaymentModel.Status.WAITING_FOR_PAYMENT
                            ]:
                                sp.status = SubjectPaymentModel.Status.PENDING
                                sp.save(update_fields=["status", "updated_at"])
                                logger.info(
                                    f"SubjectPayment {sp.id} status changed to PENDING after payment cancellation"
                                )
                    except Exception as sync_err:
                        logger.error(
                            f"Failed to sync SubjectPayment status after cancellation for payment {payment.id}: {sync_err}",
                            exc_info=True,
                        )
                elif (
                    yookassa_status == "waiting_for_capture"
                    and payment.status != Payment.Status.WAITING_FOR_CAPTURE
                ):
                    payment.status = Payment.Status.WAITING_FOR_CAPTURE
                    payment.save(update_fields=["status", "updated"])
                    # Устанавливаем статус WAITING_FOR_PAYMENT для SubjectPayment
                    try:
                        ...
                        for sp in subject_payments:
                            if sp.status in [SubjectPaymentModel.Status.PENDING]:
                                ...
                                sp.save(update_fields=["status", "updated_at"])
                                logger.info(
                                    f"SubjectPayment {sp.id} status changed to WAITING_FOR_PAYMENT"
                                )
                    except Exception as sync_err:
                        logger.error(
                            f"Failed to sync SubjectPayment status to WAITING_FOR_PAYMENT for payment {payment.id}: {sync_err}",
                            exc_info=True,
                        )
                    ...
                    try:
                        ...
                        for sp in subject_payments:
                            if sp.status == SubjectPaymentModel.Status.PENDING:
                                ...
                                sp.save(update_fields=["status", "updated_at"])
                                logger.info(
                                    f"SubjectPayment {sp.id} status changed to WAITING_FOR_PAYMENT (pending with confirmation_url)"
                                )
                    except Exception as sync_err:
                        logger.error(
                            f"Failed to sync SubjectPayment status to WAITING_FOR_PAYMENT for payment {payment.id}: {sync_err}",
...
class PaymentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if yookassa_payment and "error" in yookassa_payment:
            ...
            payment.confirmation_url = yookassa_payment.get("confirmation", {}).get(
                "confirmation_url"
            )
            payment.return_url = (
                f"{request.build_absolute_uri(SUCCESS_URL)}?payment_id={payment.id}"
            )
            ...
            if subject_payment_id or enrollment_id:
                try:
                    ...
                    payment_status = (
                        SP.Status.WAITING_FOR_PAYMENT
                        if payment.confirmation_url
                        else SP.Status.PENDING
                    )
                    ...
                    if subject_payment_id:
                        try:
                            ...
                            if (
                                ...
                                and subject_payment.status == SP.Status.PENDING
                            ):
                                subject_payment.status = payment_status
                                update_fields_list.append("status")
                            elif (
                                not payment.confirmation_url
                                and subject_payment.status != payment_status
                            ):
                                subject_payment.status = payment_status
                                update_fields_list.append("status")
                            ...
                            logger.info(
                                f"Updated SubjectPayment {subject_payment.id} with Payment {payment.id}, status: {subject_payment.status}"
                            )
                        except SP.DoesNotExist:
                            logger.warning(
                                f"SubjectPayment with id {subject_payment_id} not found"
                            )
                            ...
                            if enrollment_id:
                                try:
                                    ...
                                    logger.info(
                                        f"Created new SubjectPayment {subject_payment.id} for enrollment {enrollment_id}"
                                    )
                    ...
                    logger.error(
                        f"Failed to link Payment {payment.id} with SubjectPayment: {e}",
                        exc_info=True,
                    )
            ...
            logger.error(
                f"Unexpected: create_yookassa_payment returned None/empty for payment {payment.id}"
            )
            ...
            return Response(
                {"error": "Ошибка создания платежа в ЮКассе", "payment_id": payment.id},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def status(self, request, pk=None):
        ...
        if payment.status in [
            ...
            logger.debug(
                f"PaymentViewSet.status: Payment {payment.id} in terminal state {payment.status}, returning DB status"
            )
