...
class TutorDashboardService:
    ...
    def get_students(self) -> List[Dict[str, Any]]:
        ...
        for student in students:
            ...
            enrollments = getattr(student, "active_enrollments", [])
            subjects = [
                {
            ...
            parent = profile.parent if profile else None
    ...
    def get_student_progress(self, student_id: int) -> Dict[str, Any]:
        ...
        subject_stats_data = (
            MaterialProgress.objects.filter(
                student=student, material__subject__in=[e.subject for e in enrollments]
            )
            .values("material__subject__id", "material__subject__name")
            .annotate(
                total=Count("id"),
        ...
        subject_progress = []
        for enrollment in enrollments:
            subject_stats = stats_by_subject.get(
                enrollment.subject.id, {"total": 0, "completed": 0, "avg_progress": 0}
            )
    ...
    def create_student_report(
        self, student_id: int, parent_id: int, report_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            return {"success": False, "message": "Студент или родитель не найден"}
    ...
    def get_tutor_reports(self) -> List[Dict[str, Any]]:
        ...
        reports = (
            Report.objects.filter(author=self.tutor)
            .prefetch_related(
                Prefetch(
                    "recipients",
    ...
    def get_dashboard_data(self) -> Dict[str, Any]:
        ...
        total_subjects_assigned = SubjectEnrollment.objects.filter(
            student__student_profile__tutor=self.tutor, is_active=True
        ).count()
