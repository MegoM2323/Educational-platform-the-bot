...
class EnvConfig:
    ...
    def get_yookassa_api_url(self) -> str:
        ...
        """
        return os.getenv("YOOKASSA_API_BASE_URL", "https://api.yookassa.ru/v3")
    ...
    def get_pachca_api_url(self) -> str:
        ...
        Returns:
            str: Pachca API URL
        """
        return os.getenv("PACHCA_BASE_URL", "https://api.pachca.com/api/shared/v1")
    ...
    def get_csrf_trusted_origins(self) -> List[str]:
        """
        Get the CSRF trusted origins for this environment.

        Returns:
            List[str]: List of trusted origins for CSRF validation
        """
        if self.is_production():
            return [
                f"https://{self.PRODUCTION_DOMAIN}",
