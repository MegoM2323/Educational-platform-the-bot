...
class Notification(models.Model):
    ...
    type = models.CharField(
        max_length=30, choices=Type.choices, default=Type.SYSTEM, verbose_name="Тип уведомления"
    )

    priority = models.CharField(
        max_length=10, choices=Priority.choices, default=Priority.NORMAL, verbose_name="Приоритет"
    )
    ...
    related_object_type = models.CharField(
        max_length=50, blank=True, verbose_name="Тип связанного объекта"
    )
    ...
    # Архивирование
    is_archived = models.BooleanField(default=False, verbose_name="Архивировано", db_index=True)
...
class NotificationTemplate(models.Model):
    ...
    type = models.CharField(
        max_length=30, choices=Notification.Type.choices, verbose_name="Тип уведомления"
    )
...
class NotificationSettings(models.Model):
    ...
    assignment_notifications = models.BooleanField(
        default=True, verbose_name="Уведомления о заданиях"
    )

    material_notifications = models.BooleanField(
        default=True, verbose_name="Уведомления о материалах"
    )
    ...
    invoice_notifications = models.BooleanField(default=True, verbose_name="Уведомления о счетах")
    ...
    # Время тишины
    quiet_hours_enabled = models.BooleanField(default=False, verbose_name="Включены тихие часы")
...
class Broadcast(models.Model):
    ...
    target_group = models.CharField(
        max_length=20, choices=TargetGroup.choices, verbose_name="Целевая группа"
    )
    ...
    failed_count = models.IntegerField(default=0, verbose_name="Количество неудачных")
...
class BroadcastRecipient(models.Model):
    ...
    broadcast = models.ForeignKey(
        Broadcast, on_delete=models.CASCADE, related_name="recipients", verbose_name="Рассылка"
    )
    ...
    telegram_message_id = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="ID сообщения в Telegram"
    )

    telegram_error = models.TextField(
        null=True, blank=True, verbose_name="Ошибка отправки в Telegram"
    )
...
class NotificationClick(models.Model):
    ...
    notification = models.ForeignKey(
        Notification, on_delete=models.CASCADE, related_name="clicks", verbose_name="Уведомление"
    )
    ...
    action_data = models.JSONField(
        default=dict, blank=True, verbose_name="Дополнительные данные действия"
    )
...
class PushDeliveryLog(models.Model):
    """
    Лог отправки push-уведомлений через SMS.

    Отслеживает статус доставки push-уведомлений пользователям,
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="push_delivery_logs", verbose_name="Получатель"
    )
    ...
    # Результаты
    success = models.BooleanField(default=False, verbose_name="Успешно отправлено", db_index=True)

    error_message = models.TextField(blank=True, verbose_name="Сообщение об ошибке")

    error_code = models.CharField(
        max_length=50, blank=True, verbose_name="Код ошибки", help_text="SMS error code"
    )
    ...
    retry_at = models.DateTimeField(null=True, blank=True, verbose_name="Следующая попытка")
    ...
    def __str__(self):
        return (
            f"Push delivery {self.notification.id} to {self.user.email} "
...
class NotificationUnsubscribe(models.Model):
    ...
    channel = models.CharField(
        max_length=10, choices=Channel.choices, default=Channel.EMAIL, verbose_name="Channel"
    )
    ...
    reason = models.TextField(
        blank=True, help_text="User reason for unsubscribing (if provided)", verbose_name="Reason"
    )
