...
def check_rate_limit(request, action, limit=10, period=3600):
    """
    Проверка rate limiting для защиты от brute-force атак.

    Args:
        request: HTTP request объект
        action: Тип действия (напр. 'payment', 'subscription')
        limit: Максимальное количество запросов (по умолчанию 10)
        period: Период в секундах (по умолчанию 3600 = 1 час)

    Returns:
        True если лимит не превышен, False если превышен
    """
    key = f"ratelimit:{request.user.id}:{action}"
        ...
        try:
            ...
            if auth_header.startswith("Token "):
                ...
                try:
                    ...
                    logger.info(
                        f"[ParentDashboardView.get] Token belongs to user: {token_obj.user.username}, role: {token_obj.user.role}"
                    )
            ...
            logger.info(
                f"[ParentDashboardView.get] User role: {request.user.role}, type: {type(request.user.role)}"
            )
            logger.info(
            ...
            logger.info(
                f"[ParentDashboardView.get] Dashboard data children count: {len(dashboard_data.get('children', []))}"
            )
            logger.info(
                f"[ParentDashboardView.get] Dashboard data keys: {list(dashboard_data.keys())}"
            )
            logger.info(
                f"[ParentDashboardView.get] Children data: {dashboard_data.get('children', [])}"
            )
            ...
            logger.info(
                f"[ParentDashboardView.get] Full dashboard data (first 2000 chars): {json.dumps(dashboard_data, default=str, ensure_ascii=False)[:2000]}"
            )
...
class ParentChildrenView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        try:
            ...
            logger.info(
                f"[ParentChildrenView.list] Found {children.count()} children for parent {request.user.username}"
            )
...
def get_child_subjects(request, child_id):
    ...
    try:
        logger.info(
            f"[get_child_subjects] Request from user: {request.user.username}, role: {request.user.role}"
        )
        logger.info(
            f"[get_child_subjects] Authorization header: {request.META.get('HTTP_AUTHORIZATION', 'NOT SET')}"
        )
...
def get_child_progress(request, child_id):
    ...
    try:
        logger.info(
            f"[get_child_progress] Request from user: {request.user.username}, role: {request.user.role}"
        )
        logger.info(
            f"[get_child_progress] Authorization header: {request.META.get('HTTP_AUTHORIZATION', 'NOT SET')}"
        )
        service = ParentDashboardService(request.user)
        child = User.objects.get(id=child_id, role=User.Role.STUDENT)
...
def get_child_teachers(request, child_id):
    ...
    try:
        logger.info(
            f"[get_child_teachers] Request from user: {request.user.username}, role: {request.user.role}"
        )
        service = ParentDashboardService(request.user)
        child = User.objects.get(id=child_id, role=User.Role.STUDENT)
...
def parent_payments(request):
    ...
    try:
        logger.info(
            f"[parent_payments] Request from user: {request.user.username}, role: {request.user.role}"
        )
        logger.info(
            f"[parent_payments] Authorization header: {request.META.get('HTTP_AUTHORIZATION', 'NOT SET')}"
        )
        service = ParentDashboardService(request.user)
...
def initiate_payment(request, child_id, enrollment_id):
    ...
    """
    try:
        if not check_rate_limit(request, "payment", limit=10, period=3600):
            return Response(
...
def cancel_subscription(request, child_id, enrollment_id):
    ...
    """
    try:
        if not check_rate_limit(request, "subscription", limit=10, period=3600):
            return Response(
        ...
        try:
            enrollment = SubjectEnrollment.objects.get(
                id=enrollment_id, student=child, is_active=True
            )
        except SubjectEnrollment.DoesNotExist:
            return Response(
        ...
        try:
            subscription = SubjectSubscription.objects.get(
                enrollment=enrollment, status=SubjectSubscription.Status.ACTIVE
            )
        ...
        if subscription.enrollment.student.student_profile.parent != request.user:
            return Response(
        ...
        return Response(
            {"success": True, "message": "Подписка успешно отменена"},
            status=status.HTTP_200_OK,
        )
