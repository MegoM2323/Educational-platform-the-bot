...
class ChatRoom(models.Model):
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["type", "enrollment_id"], name="chat_type_enrollment_idx_v2"
            ),
        ]
        constraints = [
    ...
    # NOTE: last_message делает запрос к БД. Для оптимизации в view используйте:
    # annotate(last_msg_time=Max('messages__created_at', filter=Q(messages__is_deleted=False)))
    # и prefetch_related с Prefetch для сообщений
    @property
...
class Message(models.Model):
    ...
    file = models.FileField(
        upload_to="chat/files/", blank=True, null=True, verbose_name="Файл"
    )

    image = models.ImageField(
        upload_to="chat/images/", blank=True, null=True, verbose_name="Изображение"
    )
        ...
        return cls.objects.annotate(
            _annotated_unread_count=Count(
                ...
                filter=Q(
                    ...
                    room__messages__created_at__gt=F("last_read_at"),
                )
                & ~Q(room__messages__sender=F("user")),
            )
        )
