...
class SaveElementProgressView(APIView):
    ...
    def post(self, request, element_id):
        try:
            ...
            graph_lesson = GraphLesson.objects.select_related("graph", "lesson").get(
                id=graph_lesson_id
            )
            ...
            if graph_lesson.graph.student != request.user:
                return Response(
                    {
                        "success": False,
                        "error": "Вы не являетесь студентом этого графа",
                    },
            ...
            serializer = SaveElementProgressSerializer(
                data=request.data, context={"element": element}
            )
            ...
            return Response(
                {
                    "success": False,
                    "error": f"Ошибка при сохранении прогресса: {str(e)}",
                },
        ...
        try:
            ...
            if (
                ...
                return Response(
                    {
                        "success": False,
                        "error": "У вас нет прав для просмотра прогресса этого студента",
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            if graph_lesson_id:
                ...
                progress = (
                    ...
                    .select_related("element", "student")
                    .first()
                )
                ...
                progress_list = ElementProgress.objects.filter(
                    student=student, element=element
            ...
            return Response(
                {
                    "success": False,
                    "error": f"Ошибка при получении прогресса: {str(e)}",
                },
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
...
class GetLessonProgressView(APIView):
    ...
    def get(self, request, lesson_id, student_id):
        try:
            ...
            # Получить GraphLesson
            graph_lesson = GraphLesson.objects.select_related("lesson", "graph").get(
                id=graph_lesson_id
            )
            ...
            progress = (
                LessonProgress.objects.filter(
                    student=student, graph_lesson=graph_lesson
                )
                .select_related("student", "graph_lesson", "graph_lesson__lesson")
                .first()
            )
            ...
            can_start_result = PrerequisiteChecker.can_start_lesson(
                student, graph_lesson
            )
        ...
        try:
            ...
            if request.user != student:
                return Response(
                    {
                        "success": False,
                        "error": "Вы можете обновлять только свой прогресс",
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
                ...
                if new_status == "in_progress" and progress.status == "not_started":
                    ...
                    if progress.completion_percent < 100:
                        return Response(
                            {
                                "success": False,
                                "error": "Невозможно завершить урок - не все элементы выполнены",
                            },
                            status=status.HTTP_400_BAD_REQUEST,
                        )
