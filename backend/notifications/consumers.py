...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    async def disconnect(self, close_code):
        ...
        logger.info(
            ...
            f'user_id={self.user.id}, close_code={close_code}'
        )
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            logger.error(
                f'[NotificationConsumer] Invalid JSON: user_id={self.user.id}'
            )
            await self.send(text_data=json.dumps({
                'type': 'error',
            ...
            logger.error(
                f'[NotificationConsumer] Error processing message: {str(e)}'
            )
            await self.send(text_data=json.dumps({
                'type': 'error',
    ...
    def get_unread_notifications(self):
        ...
        return list(
            Notification.objects.filter(
                ...
                is_read=False,
                is_archived=False
            ).order_by('-created_at')[:50]  # Лимит 50 для производительности
        )
