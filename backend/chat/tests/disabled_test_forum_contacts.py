...
class TestAvailableContacts(TestCase):
    ...
    def test_tutor_sees_teacher_without_chat(self):
        ...
        teacher_contact = next(
            (c for c in response.data["results"] if c['id'] == self.teacher.id),
            None
        )
        ...
        self.assertIsNone(
            ...
            "chat_id should be None when no chat exists"
        )
    ...
    def test_tutor_sees_teacher_with_chat(self):
        ...
        chat = ChatRoom.objects.filter(
            type=ChatRoom.Type.FORUM_TUTOR,
            participants=self.tutor
        ).filter(participants=self.teacher).first()
    ...
    def test_tutor_sees_student_with_chat(self):
        ...
        chat = ChatRoom.objects.filter(
            type=ChatRoom.Type.FORUM_TUTOR,
            enrollment__student=self.student
        ).filter(participants=self.tutor).first()
    ...
    def test_student_sees_tutor_without_chat(self):
        ...
        tutor_contact = next(
            (c for c in response.data["results"] if c['id'] == self.tutor.id),
            None
        )
    ...
    def test_contact_fields_are_populated(self):
        ...
        required_fields = ['id', 'email', 'first_name', 'last_name', 'role', 'has_active_chat', 'chat_id']
        for field in required_fields:
            self.assertIn(field, student_contact, f"Field '{field}' should be in contact data")
    ...
    def test_contact_has_extended_fields(self):
        ...
        self.assertIsNotNone(student_contact)
        extended_fields = ['user_id', 'full_name', 'is_teacher', 'is_tutor', 'avatar', 'avatar_url']
        for field in extended_fields:
            self.assertIn(field, student_contact, f"Extended field '{field}' should be in contact data")
