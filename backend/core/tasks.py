...
def cleanup_audit_log():
    """
    Очищает старые записи лога аудита согласно политике удержания.

    По умолчанию удаляет записи старше 1 года.
    ...
    try:
        ...
        # Определяем дату отсечки (1 год назад)
        retention_days = 365  # 1 год
        ...
        if count_before > 0:
            logger.info(f"Found {count_before} audit log entries older than {cutoff_date}")

            # Удаляем старые записи в батчах для оптимизации памяти
            batch_size = 1000
            deleted_count = 0

            while old_logs.exists():
                batch = old_logs[:batch_size]
                ...
                if batch_ids:
                    ...
                    logger.debug(
                        f"Deleted {deleted_count}/{count_before} audit log entries"
                    )

            # Логируем успешное выполнение
            log_system_event(
                'audit_log_cleanup',
                f'Audit log cleanup completed: {deleted_count} entries deleted',
                'info',
                metadata={
                    'deleted_count': deleted_count,
            ...
            logger.info(f"Audit log cleanup completed: {deleted_count} entries deleted")

            return {
                'success': True,
                'deleted_count': deleted_count,
                'retention_days': retention_days,
                'message': f'Audit log cleanup completed: {deleted_count} entries deleted'
            }
        else:
            logger.info("No old audit log entries found to delete")

            return {
                'success': True,
                'deleted_count': 0,
                'message': 'No old audit log entries to delete'
            }

    except Exception as e:
        error_msg = f"Error during audit log cleanup: {str(e)}"
        logger.error(error_msg, exc_info=True)
