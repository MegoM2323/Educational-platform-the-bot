...
def create_materialized_views(apps, schema_editor):
    ...
    with schema_editor.connection.cursor() as cursor:
        ...
        cursor.execute("""
            ...
            SELECT
                se.subject_id as class_id,
                se.subject_id,
                COUNT(DISTINCT se.student_id)::INTEGER as student_count,
                ROUND(AVG(CAST(mf.grade AS DECIMAL)), 2)::DECIMAL as avg_grade,
                COUNT(DISTINCT sr.id)::INTEGER as total_submissions,
                ROUND(CAST(COUNT(CASE WHEN mf.grade >= 4 THEN 1 END) AS DECIMAL) * 100 /
                    NULLIF(COUNT(DISTINCT sr.id), 0), 2)::DECIMAL as pass_rate,
                ROUND(CAST(COUNT(DISTINCT CASE WHEN sr.submitted_at >= CURRENT_DATE - INTERVAL '7 days' THEN sr.id END) AS DECIMAL) * 100 /
                    NULLIF(COUNT(DISTINCT sr.id), 0), 2)::DECIMAL as recent_submission_rate
            FROM materials_subjectenrollment se
        ...
        cursor.execute("""
            ...
            SELECT
                ...
                COUNT(DISTINCT sr.id)::INTEGER as total_submissions,
                ROUND(AVG(CAST(mf.grade AS DECIMAL)), 2)::DECIMAL as avg_grade,
                MIN(CAST(mf.grade AS DECIMAL))::DECIMAL as min_grade,
                MAX(CAST(mf.grade AS DECIMAL))::DECIMAL as max_grade,
                ROUND(CAST(COUNT(CASE WHEN mf.grade = 5 THEN 1 END) AS DECIMAL) * 100 /
                    NULLIF(COUNT(DISTINCT sr.id), 0), 2)::DECIMAL as excellent_rate,
                ROUND(CAST(COUNT(CASE WHEN mf.grade = 4 THEN 1 END) AS DECIMAL) * 100 /
                    NULLIF(COUNT(DISTINCT sr.id), 0), 2)::DECIMAL as good_rate,
                ROUND(CAST(COUNT(CASE WHEN mf.grade <= 3 THEN 1 END) AS DECIMAL) * 100 /
            ...
            LEFT JOIN materials_materialsubmission sr ON m.id = sr.material_id
            LEFT JOIN materials_materialfeedback mf ON sr.id = mf.submission_id
            WHERE mf.grade IS NOT NULL
