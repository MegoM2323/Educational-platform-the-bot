...
class ChatRoomAdmin(admin.ModelAdmin):
    ...
    def participants_count(self, obj):
        count = getattr(obj, "_participants_count", obj.participants.count())
        return format_html(
    ...
    def messages_count(self, obj):
        count = getattr(obj, "_messages_count", obj.messages.count())
        return format_html(
...
class MessageAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Файлы", {"fields": ("file", "image"), "classes": ("collapse",)}),
        (
            "Дополнительно",
            {"fields": ("reply_to", "is_edited"), "classes": ("collapse",)},
        ),
        (
            "Временные метки",
            {"fields": ("created_at", "updated_at"), "classes": ("collapse",)},
...
class MessageReadAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ("Основная информация", {"fields": ("message", "user")}),
        ("Временные метки", {"fields": ("read_at",)}),
    )
...
class ChatParticipantAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        ("Права и настройки", {"fields": ("is_admin", "is_muted")}),
        ("Временные метки", {"fields": ("joined_at", "last_read_at")}),
        ("Статистика", {"fields": ("unread_count_display",)}),
    )

    def get_queryset(self, request):
        """Используем with_unread_count() для избежания N+1"""
        return ChatParticipant.with_unread_count().select_related("room", "user")
