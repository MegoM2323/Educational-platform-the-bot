...
def process_successful_payment(payment):
    ...
    try:
        logger.info(
            f"[Transaction START] Beginning atomic transaction for payment {payment.id}"
        )
        ...
        with transaction.atomic():
            ...
            if payment.status != Payment.Status.SUCCEEDED:
                ...
                payment.save(
                    update_fields=["status", "paid_at", "raw_response", "updated"]
                )
                ...
                if payment.status == Payment.Status.SUCCEEDED:
                    logger.info(
                        f"✅ VERIFIED: Payment {payment.id} status=SUCCEEDED in DB. Metadata: {payment.metadata}"
                    )
                else:
                    error_msg = (
            ...
            logger.info(
                f"Found {subject_payments.count()} subject payments for payment {payment.id}"
            )
            ...
            if subject_payments.count() == 0 and isinstance(payment.metadata, dict):
                ...
                if enrollment_id:
                    try:
                        ...
                        logger.error(
                            f"Enrollment {enrollment_id} not found in metadata for orphaned payment {payment.id}"
                        )
                        result["errors"].append(
                            f"Enrollment {enrollment_id} not found for orphaned payment"
                        )
                        return result
                    except Exception as e:
                        logger.error(
                            f"Failed to create orphaned SubjectPayment for payment {payment.id}: {e}",
                            exc_info=True,
                        )
                        result["errors"].append(
                            f"Failed to create orphaned SubjectPayment: {e}"
                        )
                        return result
                else:
                    logger.error(
                        f"No enrollment_id in metadata for orphaned payment {payment.id}. Metadata: {payment.metadata}"
                    )
                    result["errors"].append(
                        "No enrollment_id found in payment metadata for orphaned payment"
                    )
                    return result
                ...
                try:
                    ...
                    if subject_payment.status != SP.Status.PAID:
                        ...
                        if subject_payment.status == SP.Status.PAID:
                            ...
                            logger.info(
                                f"✅ VERIFIED: SubjectPayment {subject_payment.id} status=PAID in DB"
                            )
                        else:
                            error_msg = (
                    ...
                    logger.info(
                        f"Enrollment {enrollment.id} BEFORE activation: is_active={enrollment.is_active}"
                    )
                    if not enrollment.is_active:
                        ...
                        if enrollment.is_active:
                            ...
                            logger.info(
                                f"✅ VERIFIED: Enrollment {enrollment.id} is_active=True in DB after payment {payment.id}"
                            )
                        else:
                    ...
                    subscription_processed = _process_subscription_for_payment(
                        payment, subject_payment, enrollment
                    )
                    if subscription_processed:
                        result["subscriptions_processed"] += 1
                        logger.info(
                            f"Subscription processed successfully for enrollment {enrollment.id}"
                        )
                    else:
                        logger.warning(
                            f"Subscription NOT processed for enrollment {enrollment.id}"
                        )
                    ...
                    logger.info(
                        f"Added notification to queue for SubjectPayment {subject_payment.id}"
                    )
        ...
        logger.info(
            f"[Transaction COMMITTED] All database changes committed for payment {payment.id}"
        )
    ...
    if result["success"] and notifications_to_send:
        logger.info(
            f"[Notifications] Starting notification sending for {len(notifications_to_send)} payments (outside transaction)"
        )
        ...
        for subject_payment in notifications_to_send:
            try:
                ...
                if notification_sent:
                    ...
                    logger.info(
                        f"[Notifications] Successfully sent notification for SubjectPayment {subject_payment.id}"
                    )
                else:
                    logger.warning(
                        f"[Notifications] Failed to send notification for SubjectPayment {subject_payment.id} (non-critical)"
                    )
        ...
        logger.info(
            f"[Notifications] Completed notification sending. Sent: {result['notifications_sent']}/{len(notifications_to_send)}"
        )

    logger.info(
        f"=== Completed process_successful_payment for payment {payment.id} ==="
    )
...
def _process_subscription_for_payment(payment, subject_payment, enrollment):
    ...
    create_subscription = (
        payment.metadata.get("create_subscription", False)
        if isinstance(payment.metadata, dict)
        else False
    )
    is_recurring = (
        payment.metadata.get("is_recurring", False)
        if isinstance(payment.metadata, dict)
        else False
    )
    ...
    try:
        ...
        try:
            ...
            if (
                subscription.status == SubjectSubscription.Status.ACTIVE
                and subscription.amount == subject_payment.amount
            ):
                logger.info(
...
def _send_payment_notification(subject_payment):
    ...
    try:
        ...
        if parent:
            ...
            logger.info(
                f"Sent payment notification to parent {parent.id} for payment {subject_payment.payment.id}"
            )
            return True
        else:
            logger.warning(
                f"No parent found for student {student.id}, skipping notification"
            )
            return False
...
def sync_payment_status(payment, yookassa_payment_data):
    ...
    if new_status and payment.status != new_status:
        ...
        if new_status == Payment.Status.SUCCEEDED and not payment.paid_at:
            payment.paid_at = timezone.now()
            payment.save(update_fields=["status", "paid_at", "raw_response", "updated"])
        else:
...
def create_invoice_payment(invoice, user, request):
    ...
    try:
        ...
        logger.error(
            f"Failed to create Payment record for invoice {invoice.id}: {e}",
            exc_info=True,
        )
    ...
    if yookassa_payment and "error" in yookassa_payment:
        ...
        logger.error(
            f"Unexpected: create_yookassa_payment returned None for invoice {invoice.id}"
        )
...
def mark_invoice_paid(invoice_id, payment):
    ...
    try:
        ...
        logger.error(
            f"Unexpected error marking invoice {invoice_id} as paid: {e}", exc_info=True
        )
