...
class LessonListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        queryset = Lesson.objects.select_related('created_by', 'subject').prefetch_related('elements').filter(
...
class LessonRetrieveUpdateDestroyView(generics.RetrieveUpdateDestroyAPIView):
    ...
    """
    queryset = Lesson.objects.select_related('created_by', 'subject').prefetch_related('elements')
    ...
    def retrieve(self, request, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error in LessonRetrieveUpdateDestroyView.retrieve: {e}", exc_info=True)
            return Response(
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            if instance.created_by != request.user and not request.user.is_staff:
                return Response(
                    {'success': False, 'error': 'Только владелец может редактировать урок'},
            ...
            logger.error(f"Error in LessonRetrieveUpdateDestroyView.update: {e}", exc_info=True)
            return Response(
    ...
    def destroy(self, request, *args, **kwargs):
        ...
        try:
            ...
            if instance.graphs.exists():
                return Response(
                    {'success': False, 'error': 'Урок используется в графах знаний и не может быть удален'},
                    status=status.HTTP_400_BAD_REQUEST
                )

            self.perform_destroy(instance)
            return Response(
                {'success': True, 'message': 'Урок успешно удален'},
            ...
            logger.error(f"Error in LessonRetrieveUpdateDestroyView.destroy: {e}", exc_info=True)
            return Response(
    ...
    def post(self, request, lesson_id):
        try:
            ...
            if lesson.created_by != request.user and not request.user.is_staff:
                return Response(
                    {'success': False, 'error': 'Только владелец может добавлять элементы в урок'},
                    status=status.HTTP_403_FORBIDDEN
                )
            ...
            if LessonElement.objects.filter(lesson=lesson, element=element).exists():
                return Response(
            ...
            return Response(
                {'success': False, 'error': f'Ошибка при добавлении элемента: {str(e)}'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
...
class RemoveElementFromLessonView(APIView):
    ...
    def delete(self, request, lesson_id, element_id):
        try:
            ...
            if lesson.created_by != request.user and not request.user.is_staff:
                return Response(
                    {'success': False, 'error': 'Только владелец может удалять элементы из урока'},
                    status=status.HTTP_403_FORBIDDEN
                )
            ...
            return Response(
                {'success': True, 'message': 'Элемент успешно удален из урока'},
                status=status.HTTP_204_NO_CONTENT
            )
