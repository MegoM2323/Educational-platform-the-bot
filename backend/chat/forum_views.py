...
class AvailableContactsView(APIView):
    ...
    def _build_contact_to_chat_mapping(self, user: User) -> tuple:
        ...
        try:
            ...
            logger.error(
                f"[AvailableContactsView._build_contact_to_chat_mapping()] Database error for user {user.id}: {str(e)}",
                exc_info=True,
            )
            raise
    ...
    def get(self, request: Request) -> Response:
        ...
        try:
            ...
            # Обернуть вызов в отдельный try-except для безопасной обработки database errors
            try:
                (
                    contact_to_chat,
                    enrollment_to_chat,
                ...
                if tutor_ids:
                    for enrollment in SubjectEnrollment.objects.filter(
                        ...
                        student_profile = getattr(
                            enrollment.student, "student_profile", None
                        )
            ...
            logger.error(
                f"[AvailableContactsView.get()] Error fetching contacts for user {user.id}: {str(e)}",
                exc_info=True,
            )
            return Response(
                {
