...
class MetricsCollector:
    ...
    def collect_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"timestamp": timestamp_str, "error": str(e), "status": "error"}
    ...
    def _store_metrics(self, metrics: Dict[str, Any]) -> None:
        ...
        try:
            ...
            cache.set(
                cache_key, json.dumps(metrics), timeout=7 * 24 * 3600
            )  # 7 days TTL
    ...
    def _get_cpu_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "avg_5min_percent": round(load_avg[0] / cpu_count * 100, 2)
                if cpu_count
                else 0,
                "avg_15min_percent": round(load_avg[2] / cpu_count * 100, 2)
                if cpu_count
                else 0,
    ...
    def _get_database_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                if response_time_ms < self.ALERT_THRESHOLDS["db_latency"]
                else "critical",
        ...
        try:
            ...
            return {
                ...
                "error_rate_percent": round(
                    ...
                    / max(cache.get("api_total_requests", 1), 1)
                    * 100,
                    2,
                )
                if cache.get("api_total_requests", 0) > 0
                else 0,
                "status": "healthy"
                if int(errors_5xx) == 0
                else "warning"
                if int(errors_5xx) < 10
                else "critical",
            }
        except Exception as e:
    ...
    def _get_latency_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "status": "healthy"
                if (sorted_times[int(n * 0.95)] if n > 0 else 0)
...
class AlertSystem:
    ...
    def check_thresholds(self, metrics: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        with self._lock:
            ...
            if disk_data.get("status") == "critical":
                partitions_info = ", ".join(
                    [p["partition"] for p in disk_data.get("partitions", [])]
                )
                self._create_alert(
                    "disk",
                    "critical",
            ...
            if redis_data.get("status") == "critical":
                self._create_alert(
                    "redis", "critical", "Redis connection failed", redis_data, alerts
                )
            else:
    ...
    def _create_alert(
        self, component: str, severity: str, message: str, data: Dict, alerts: List
    ) -> None:
