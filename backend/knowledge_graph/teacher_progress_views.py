...
class GraphProgressOverviewView(APIView):
    ...
    def get(self, request, graph_id):
        try:
            # Получить граф
            graph = KnowledgeGraph.objects.select_related("created_by", "student", "subject").get(
                id=graph_id
            )
            ...
            if graph.created_by != request.user and not request.user.is_staff:
                return Response(
                    {
                        "success": False,
                        "error": "Только создатель графа может просматривать прогресс",
                    },
            ...
            lesson_progress_list = LessonProgress.objects.filter(
                student=student, graph_lesson__graph=graph
            ...
            max_possible_score = (
                lesson_progress_list.aggregate(total=Sum("max_possible_score"))["total"] or 0
            )
            ...
            return Response(
                {"success": False, "error": "Граф не найден"}, status=status.HTTP_404_NOT_FOUND
            )
...
class LessonDetailView(APIView):
    ...
    def get(self, request, graph_id, student_id, lesson_id):
        try:
            ...
            graph_lesson = (
                GraphLesson.objects.filter(graph=graph, lesson_id=lesson_id)
                .select_related("lesson")
                .first()
            )
            ...
            lesson_elements = (
                LessonElement.objects.filter(lesson=graph_lesson.lesson)
                .select_related("element")
                .prefetch_related(
                    Prefetch(
                        "element__student_progress",
                        queryset=ElementProgress.objects.filter(
                            student=graph.student, graph_lesson=graph_lesson
                        ),
                        to_attr="student_progress_list",
                    )
                )
            ...
            for le in lesson_elements:
                # Получить прогресс из prefetch (без дополнительных запросов)
                progress = (
                    le.element.student_progress_list[0]
                    if le.element.student_progress_list
                    else None
                )
        ...
        try:
            ...
            if graph.created_by != request.user and not request.user.is_staff:
                return Response(
                    {
                        "success": False,
                        "error": "Только создатель графа может экспортировать прогресс",
                    },
                    status=status.HTTP_403_FORBIDDEN,
                )
            ...
            # Получить все уроки и прогресс (FIX N+1: prefetch LessonProgress)
            from django.db.models import Prefetch
                ...
                if progress:
                    writer.writerow(
                        [
                            ...
                            progress.started_at.strftime("%Y-%m-%d %H:%M")
                            if progress.started_at
                            else "",
                            progress.completed_at.strftime("%Y-%m-%d %H:%M")
                            if progress.completed_at
                            else "",
                            round(progress.total_time_spent_seconds / 60, 2)
                            if progress.total_time_spent_seconds
                            else 0,
                        ]
                    )
                else:
            ...
            response[
                "Content-Disposition"
            ] = f'attachment; filename="progress_graph_{graph_id}.csv"'
