...
class InvoiceService:
    ...
    def create_invoice(
        ...
        if not hasattr(student, 'student_profile') or not student.student_profile.parent:
    ...
    def get_tutor_invoices(
        ...
        """
        if tutor.role != 'tutor':
            raise InvoicePermissionDenied('Только тьюторы могут просматривать свои счета')
        ...
        queryset = Invoice.objects.filter(tutor=tutor).select_related(
            ...
            Prefetch(
                'status_history',
                queryset=InvoiceStatusHistory.objects.select_related('changed_by').order_by('-changed_at')
            )
        )
    ...
    def get_parent_invoices(
        ...
        """
        if parent.role != 'parent':
            raise InvoicePermissionDenied('Только родители могут просматривать свои счета')

        # Базовый queryset с оптимизацией
        queryset = Invoice.objects.filter(parent=parent).select_related(
            'tutor',
    ...
    def cancel_invoice(invoice_id: int, user: User, reason: str = '') -> Invoice:
        ...
        """
        try:
            invoice = Invoice.objects.select_related('tutor', 'student', 'parent').get(id=invoice_id)
        except Invoice.DoesNotExist:
            raise InvoiceNotFound(f'Счет с ID {invoice_id} не найден')
