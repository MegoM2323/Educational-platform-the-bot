...
class NotificationClickAdmin(admin.ModelAdmin):
    """
    Админка для отслеживания кликов по уведомлениям
    """
    list_display = [
        'notification_info', 'user', 'action_type_badge', 'created_at'
    ]
    list_filter = ['action_type', 'created_at']
    search_fields = ['notification__title', 'user__username', 'user__email', 'action_url']
    readonly_fields = ['created_at', 'notification', 'user']

    fieldsets = (
        ('Основная информация', {
            'fields': ('notification', 'user', 'action_type', 'created_at')
        }),
        ('Детали действия', {
            'fields': ('action_url', 'action_data', 'user_agent', 'ip_address')
        }),
    )
    ...
    notification_info.short_description = 'Уведомление'

    def action_type_badge(self, obj):
        """Отображает тип действия с цветным бейджем"""
        colors = {
            'link_click': 'blue',
            'in_app_click': 'green',
            'email_click': 'orange',
        ...
        emoji = emojis.get(obj.action_type, '🖱️')

        return format_html(
            '<span style="background-color: {}; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px;">{}</span>',
            color,
            f"{emoji} {obj.get_action_type_display()}"
        )
    action_type_badge.short_description = 'Тип действия'

@admin.register(NotificationUnsubscribe)
class NotificationUnsubscribeAdmin(admin.ModelAdmin):
    """
    Admin interface for tracking user unsubscribe requests (GDPR compliance).
    """
    list_display = [
        'user', 'channel_badge', 'notification_types_display',
        'token_used_badge', 'unsubscribed_at', 'resubscribed_at'
    ]
    list_filter = ['channel', 'token_used', 'unsubscribed_at', 'resubscribed_at']
    search_fields = ['user__username', 'user__email', 'reason']
    readonly_fields = ['user', 'unsubscribed_at', 'ip_address', 'user_agent']
    
    fieldsets = (
        ('User Information', {
            'fields': ('user', 'unsubscribed_at', 'resubscribed_at')
        }),
        ('Unsubscribe Details', {
            'fields': ('notification_types', 'channel', 'reason')
        }),
        ('Audit Trail', {
            'fields': ('token_used', 'ip_address', 'user_agent'),
            'classes': ('collapse',)
        }),
    )
    
    def notification_types_display(self, obj):
        """Display notification types as readable list"""
        if not obj.notification_types:
            return format_html('<span style="color: gray;">All channels</span>')
        
        types = ', '.join(obj.notification_types) if isinstance(obj.notification_types, list) else str(obj.notification_types)
        return types
    notification_types_display.short_description = 'Types'
    
    def channel_badge(self, obj):
        """Display channel with color-coded badge"""
        colors = {
            'email': 'blue',
            'push': 'green',
            'sms': 'orange',
        ...
        return format_html(
            '<span style="background-color: {}; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px;">{}</span>',
            color,
            f"{emoji} {obj.get_channel_display()}"
        )
    channel_badge.short_description = 'Channel'
    
    def token_used_badge(self, obj):
        """Display if unsubscribe was via secure token"""
        if obj.token_used:
            return format_html(
                '<span style="background-color: green; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px;">✅ Token</span>'
            )
        else:
            return format_html(
                '<span style="background-color: orange; color: white; padding: 2px 8px; border-radius: 4px; font-size: 12px;">🔗 Manual</span>'
            )
    token_used_badge.short_description = 'Method'
