...
class SupabaseSyncService:
    ...
    def sync_user_to_supabase(self, django_user: User) -> Dict[str, Any]:
        ...
        try:
            ...
            result = supabase_auth_service.update_user_profile(str(django_user.id), profile_data)

            if result["success"]:
                # Синхронизируем роли
                self._sync_user_roles(django_user)
                return {"success": True, "message": "Пользователь синхронизирован с Supabase"}
            else:
                return {"success": False, "error": result.get("error", "Ошибка синхронизации")}

        except Exception as e:
            return {"success": False, "error": f"Ошибка синхронизации: {str(e)}"}
    ...
    def _sync_user_roles(self, django_user: User) -> bool:
        ...
        try:
            # Удаляем старые роли
            supabase_auth_service.service_client.table("user_roles").delete().eq(
            ...
            logger.error(f"[supabase_sync] Ошибка синхронизации ролей: {e}")
            return False
    ...
    def create_django_user_from_supabase(self, supabase_user_data: Dict[str, Any]) -> Optional[User]:
        ...
        try:
            ...
            logger.info(f"[create_django_user_from_supabase] Role from Supabase: {role_str}, mapped to: {role}")
            ...
            django_user = User.objects.create_user(
                ...
                if profile.get("full_name") and len(profile.get("full_name", "").split(" ")) > 1
                else "",
            ...
            logger.info(
                f"[create_django_user_from_supabase] User created: {django_user.username}, role: {django_user.role}"
            )
            ...
            logger.error(f"[create_django_user_from_supabase] Ошибка создания пользователя Django: {e}")
            return None
    ...
    def _create_user_profile(self, user: User, role: str) -> None:
        ...
        try:
            ...
            logger.info(f"[_create_user_profile] Creating profile for user {user.username}, role: {role_str}")

            if role_str == "student" or role == User.Role.STUDENT:
                from .models import StudentProfile

                StudentProfile.objects.get_or_create(user=user, defaults={"grade": "1", "goal": ""})
            elif role_str == "teacher" or role == User.Role.TEACHER:
                from .models import TeacherProfile

                TeacherProfile.objects.get_or_create(
                    user=user, defaults={"subject": "Не указан", "experience_years": 0, "bio": ""}
                )
            elif role_str == "tutor" or role == User.Role.TUTOR:
                from .models import TutorProfile

                TutorProfile.objects.get_or_create(
                    user=user, defaults={"specialization": "Не указана", "experience_years": 0, "bio": ""}
                )
            ...
            logger.error(f"[_create_user_profile] Ошибка создания профиля: {e}")
    ...
    def sync_all_users(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "synced_count": synced_count, "total_count": users.count(), "errors": errors}

        except Exception as e:
            return {"success": False, "error": f"Ошибка массовой синхронизации: {str(e)}"}
